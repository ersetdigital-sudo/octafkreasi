...
// ═══════════════════════════════════════════════════════════════════════════════
// NIK SECURE INPUT COMPONENT
// ═══════════════════════════════════════════════════════════════════════════════
function NikInput({ id, value, onChange }: { id: string; value: string; onChange: (e: React.ChangeEvent<HTMLInputElement>) => void }) {
  ...
  return (
    <div className="relative">
      <input
        ...
        style={{ letterSpacing: showFull ? 'normal' : '2px' }}
      />
      <button
        type="button"
        onClick={() => setShowFull(!showFull)}
        className="absolute right-3 top-1/2 -translate-y-1/2 flex h-8 w-8 items-center justify-center rounded-lg text-gray-400 transition-colors hover:bg-gray-100 hover:text-gray-600"
        aria-label={showFull ? 'Sembunyikan NIK' : 'Tampilkan NIK'}
        tabIndex={-1}
      >
        {showFull ? (
                ...
                <div className="grid gap-4 sm:grid-cols-2">
                  ...
                  <div className="flex flex-col gap-1.5">
                    ...
                    <div className="flex">
                      ...
                      <input id="contact-wa" type="tel" inputMode="numeric" value={contactWhatsapp} onChange={(e) => setContactWhatsapp(e.target.value)} placeholder="812XXXXXXXX" autoComplete="off" className="w-full rounded-r-xl border border-gray-300 px-4 py-3 text-base min-h-[48px] text-gray-900 placeholder:text-gray-400 transition-all duration-200 focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-500/20" />
                    </div>
                  </div>
                  <div className="flex flex-col gap-1.5">
                          ...
                          <div className="border-t border-gray-100 px-4 pb-4 pt-4">
                            <div className="grid gap-4 sm:grid-cols-2">
                              <div className="flex flex-col gap-1.5 sm:col-span-2">
                                ...
                                <input id={`p-${index}-name`} type="text" value={participant.fullName} onChange={(e) => handleParticipantChange(index, 'fullName', e.target.value)} placeholder="Sesuai KTP/Paspor" className={inputClass} autoComplete="off" />
                              </div>
                              <div className="flex flex-col gap-1.5 sm:col-span-2">
                                <label htmlFor={`p-${index}-id`} className="text-sm font-medium text-gray-700">
                                  No. KTP<span className="ml-0.5 text-red-500">*</span>
                        ...
                        >
                          ...
                          <div className="bg-white p-3 text-left">
                            ...
                            <div className="mt-1 flex items-center gap-1">
                              ...
                              <span className="text-[10px] text-yellow-500">★ {addon.rating.toFixed(1)}</span>
                            </div>
